...
/* I2C bus the BME280 sits on — cached so sensors_read() can re-write the
...
static int sensors_init(void)
{
	...
	/* I2C bus reference — used in sensors_read() to re-arm CTRL_HUM
...
/* ========================================================================
 * InfluxDB uplink — push readings to the LAN database
 * ========================================================================
 *
 * Each minute (after a successful SNTP sync) we capture a sample into a
 * RAM ring buffer. A drain helper runs on every main-loop tick and tries
 * to POST the oldest pending sample to InfluxDB 1.x line protocol over
 * plain HTTP. On failure the drain stops; the next tick retries from the
 * same oldest sample, so a temporary network/NAS outage backfills cleanly
 * once connectivity returns. Buffer is RAM-only (no NVS) and holds ~6 h
 * worth of readings.
 *
 * Hardcoded LAN-only assumptions: no TLS, no auth. Both fine inside a
...
/* Drain the ring on every tick, one sample at a time, stopping on the
 * first error so we don't hammer a flapping NAS. */
static void influx_drain(void)
{
	while (influx_buf_count > 0 && wifi_connected) {
...
int main(void)
{
	 ...
	 * sensors_read() owns the pin and power-cycles it per minute-tick. */
